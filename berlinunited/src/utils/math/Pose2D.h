...
public:
	 ...
	 */
	Pose2T(Radian rotation, const Vector2<T>& translation)
		: rotation(rotation)
	 ...
	 */
	Pose2T(Radian rotation, const T x, const T y)
		: rotation(rotation)
		, translation(x, y)
	{
	}
	 ...
	 * \param translation translation (Vector2)
	 */
	Pose2T(const Vector2<T>& translation)
		: rotation(0*radians)
		, translation(translation)
	{
	}
	 ...
	 */
	template<class T2>
	Pose2T(const Vector2<T2>& translation)
		: rotation(0*radians)
	...
	Vector2<T> operator*(const Vector2<T>& point) const {
		...
		return (Vector2<T>(point.x * c - point.y * s, point.x * s + point.y * c)
				+ translation);
	}
	 ...
	 */
	Pose2T& rotate(const Radian angle) {
		rotation += angle;
		rotation = Math::normalize(rotation);
		return *this;
	}
	 ...
	 */
	Pose2T invert() const {
		const Radian invRotation = -rotation;
		return Pose2T(invRotation,
				(Vector2<T>() - translation).rotate(invRotation));
	}
