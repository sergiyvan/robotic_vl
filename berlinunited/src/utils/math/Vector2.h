...
public:
	 ...
	 * (changes the object)
	 */
	Vector2<V>& rotate(Degree angle) {
		return rotate(Radian(angle));
	}
	 ...
	 */
	Radian angle() const {
		return std::atan2(static_cast<double>(y), static_cast<double>(x)) * radians;
	}
};
