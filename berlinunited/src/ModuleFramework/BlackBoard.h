...
public:
	...
	virtual ~BlackBoard() {
		// delete the registry
		for (Registry::iterator iter = registry.begin(); iter != registry.end();
				iter++) {
			delete iter->second;
		}
	}
	...
	const T& getRepresentation(const std::string& name) const {
		...
		if (iter == registry.end()) {
			// TODO: throw
			std::cerr << "Representation not existing: " << name << " of type "
			          << typeid(T).name()
		...
		if (typedData == NULL) {
			// TODO: throw
			std::cerr << "Representation type mismatch: " << name
			          << " is registered as " << data->getTypeName() << ", but "
			          << typeid(T).name() << " is requested." << std::endl;
			assert(false);
		} //end if
