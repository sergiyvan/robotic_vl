...
REGISTER_CAMERA("darwin", CameraDARwIn, "Logitech c905 as built into DARwIn OP via v4l2");


/*------------------------------------------------------------------------------------------------*/

/** The following applies to the Logitech C905 camera with UVC interface.
 **
...
CameraDARwIn::CameraDARwIn() {
	...
	addSupportedSetting( CAMERA_FOCUS,              V4L2_CID_FOCUS_ABSOLUTE,             "Focus",                           68,     0,    255 );
...
/** Capture an image
 **
 ** Basic functionality is in CameraV4L2, in this code we only make sure we
 ** rotate the image as the camera is built into the DARwIn OP upside-down.
 */

bool CameraDARwIn::capture() {
	if (CameraV4L2::capture()) {
		image->rotate180();
		return true;
	} else {
		return false;
