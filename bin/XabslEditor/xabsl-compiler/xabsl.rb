...
require 'fileutils'
require 'vcproj'
#require 'ftools' #not required in ruby 1.9 / not available
		...
		def write_intermediate_code(ccs, agent_cc, filename)
			...
			File.open(filename, 'wb') do |file|
				...
				agent_cc.st.each do |symbol|
					case symbol
					when BooleanOutputSymbol, DecimalOutputSymbol, EnumOutputSymbol then
						if symbol.internal
							internal_output_symbols << symbol 
						end
					when EnumSymbol then
					  if symbol.internal
  			...
  			if write[:xabsl]
				...
				if message_count[CompilerError] == 0
					...
					if write[:xabsl]
						...
						if File.file?(filename) && FileUtils.compare_file(filename+".new",filename)
