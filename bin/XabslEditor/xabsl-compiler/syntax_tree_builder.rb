module Translator
    ...
    def output_symbol_assignment_early val
		...
		def output_symbol_assignment val 
			...
			when DecimalOutputSymbol then
				tag = SDOSTag.new
			when BooleanOutputSymbol then
				tag = SBOSTag.new
			when EnumOutputSymbol then
        pc.enum_domains.pop
			  #domain checking
    ...
    def exp_binary_early val
      ...
      if pc.enum_domains.last.nil?
        symbol = cc.sc.from_code(val[0], [ConstantSymbol, OptionParameterSymbol, DecimalInputSymbol, BooleanInputSymbol, EnumInputSymbol, DecimalOutputSymbol, BooleanOutputSymbol, EnumOutputSymbol])
			...
			case symbol #the tag that defined the symbol name
			when DecimalInputSymbol then
				tag = DISRTag.new
				tag.attr[ATTR_REF] = symbol
			when BooleanInputSymbol then
				tag = BISRTag.new
				tag.attr[ATTR_REF] = symbol
			when EnumInputSymbol then
				tag = EISRTag.new
				tag.attr[ATTR_REF] = symbol
			...
			when EnumOutputSymbol then
				...
				tag.attr[ATTR_REF] = symbol
				tag.enum_domain = symbol.enum_domain
			when ConstantSymbol then
				tag = CRTag.new
				tag.attr[ATTR_REF] = symbol
			when OptionParameterSymbol then
				tag = OPRTag.new
				tag.attr[ATTR_REF] = symbol
			...
			when EnumSymbolValueSymbol then
				tag = EVTag.new
				tag.attr[ATTR_REF] = symbol
			  tag.enum_domain = symbol.enum_domain
			else
