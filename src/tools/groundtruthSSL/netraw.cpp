...
void Address::print(FILE* out) const
{
	...
	const sockaddr_in* sockname = (const sockaddr_in*)(&addr);
	if (sockname->sin_family == AF_INET) {
...
bool UDP::open(int port, bool share_port_for_multicasting, bool multicast_include_localhost, bool blocking)
{
	...
	if (ret != 0) {
		fprintf(stderr, "ERROR %d WHEN SETTING IP_MULTICAST_TTL\n", ret);
		return false;
	}
	...
	if (port != 0) {
		...
		sockname.sin_port = htons(port);
		if (-1 == ::bind(fd, (struct sockaddr*)(&sockname), sizeof(sockname))) {
			fprintf(stderr, "ERROR %d (%s) WHEN BINDING PORT\n", errno, strerror(errno));
		}
	}
